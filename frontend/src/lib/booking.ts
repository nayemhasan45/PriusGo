...
export function dateRangeOverlapsBlocks(startDate: string, endDate: string, blocks: DateBlock[]): boolean {
  return blocks.some((block) => startDate <= block.endDate && endDate >= block.startDate);
}
