...
- Sprint 5 trust/conversion work is now in the workspace:
  - trust section added on homepage
  - contact wording made concrete
  - metadata/open graph improved
  - FAQ schema added
  - tests/lint/build pass with these Sprint 5 changes
- Sprint 5 printable summary work is now in the workspace:
  - printable booking summary added for admin bookings
  - contract draft / handoff sheet is generated for pickup use
  - tests/lint/build pass with these Sprint 5 changes
...
**Why now:** Sprint 5 is implemented, and the only remaining repo-level cleanup is the corrected overlap SQL in Supabase.

**Main gaps right now:**
- corrected overlap-protection SQL should be re-applied in Supabase after final review

### Priority B — Start V2.1 responsive polish
**Why now:** The app is functional; the next step is making it consistently good on phone, tablet, laptop, and desktop.

**Main gaps right now:**
- header/mobile nav can still be audited at smaller widths
- admin/public pages need another responsive smoke check

### Priority C — Start V2.2 deployment readiness
**Why now:** Once responsive polish is checked, the deployment checklist should be verified against production Supabase and Vercel.

**Main gaps right now:**
- production Supabase schema confirmation
- Auth redirect / OAuth smoke tests in production
...
8. **Printable booking summary / contract draft** ✅ done
...
**Start V2.1 responsive polish:**
- audit homepage at phone/tablet/laptop/desktop widths
- tighten header/mobile nav and page spacing where needed
- smoke-test the main pages on narrow screens
- keep overlap-protection SQL fix ready for Supabase review

Why this first:
- most of the value is already built locally
- fastest path to a real business improvement with minimal extra work
 - the MVP workflow is done, so the next win is responsive reliability
 - smallest useful step before broader V2 work
 - keeps deployment and trust work honest on real devices
